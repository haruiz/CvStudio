...
if __name__ == '__main__':
    ...
    net = resnet101(1, nInputChannels=4, classifier='psp')
    state_dict_checkpoint = torch.load(modelName + '.pth',map_location=lambda storage, loc: storage)
    ...
    image = np.array(Image.open(r"D:\pollen.jpg"))
    plt.ion()
    plt.axis('off')
    plt.imshow(image)
    ...
    with torch.no_grad():
        while 1:
            ...
            bbox = get_bbox(image, points=extreme_points_ori, pad=pad, zero_pad=True)
            ...
            result = crop2fullmask(pred, bbox, im_size=image.shape[:2], zero_pad=True, relax=pad) > thres

            results.append(result)

            # Plot the results
            plt.imshow(overlay_masks(image / 255, results))
