...
class LabelsTable(QTableView, QObject):
    action_click = Signal(QAction)
    CTX_MENU_ADD_LABEL = "Add Label"
    CTX_MENU_DELETE_LABEL = "Delete Label"

    def __init__(self, parent=None):
        super(LabelsTable, self).__init__(parent)
    ...
    def add_row(self, name, color):
        row = self._model.rowCount(QModelIndex())
        self._model.insertRow(row)
        self._model.setData(self._model.index(row, 0), name)
        self._model.setData(self._model.index(row, 1), color)
    ...
    def contextMenuEvent(self, evt: QContextMenuEvent) -> None:
        ...
        if index.isValid():
            icon: QIcon = GUIUtils.get_icon("delete_label.png")
            action = menu.addAction(icon, self.CTX_MENU_DELETE_LABEL)
            action.setData(index)
        else:
            icon: QIcon = GUIUtils.get_icon("new_label.png")
