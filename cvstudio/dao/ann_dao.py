...
from cvstudio.dao import db, AnnotationEntity, LabelEntity, DatasetEntryEntity
from cvstudio.vo import AnnVO, LabelVO
...
class AnnDao:
    ...
    def save(self, entity_id, entry: typing.Any):
        if isinstance(entry, AnnVO):
            vo = entry
            return AnnotationEntity.create(
                entry=vo.entry, label=vo.label, points=vo.points, kind=vo.kind
            )
            ...
            with db.atomic():
                ...
                query.execute()
                rows = [(vo.entry, vo.label, vo.points, vo.kind) for vo in entry]
                # AnnotationEntity \
                #     .insert_many(rows, fields=["entry", "label", "points", "kind"]) \
                #     .execute()
                for batch in chunked(rows, 100):
                    AnnotationEntity.insert_many(
                        batch, fields=["entry", "label", "points", "kind"]
                    ).execute()

    @db.connection_context()
    def delete(self, entity_id: int):
        query = AnnotationEntity.delete().where(AnnotationEntity.entry == entity_id)
        return query.execute()

    @db.connection_context()
    def fetch_all_by_image(self, entity_id: int):
    ...
    @db.connection_context()
    def get_image_label(self, entity_id: int):
    ...
    def fetch_labels(self, dataset_id: int = None):
        ...
        query = (
            e.select(e.file_path.alias("image"), l.name.alias("label"))
